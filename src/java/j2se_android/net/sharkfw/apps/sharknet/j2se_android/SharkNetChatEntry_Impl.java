...
public class SharkNetChatEntry_Impl extends InMemoSubSpaceEntry implements SharkNetChatEntry {
    ...
    // TODO: wo wird diese Methode gerufen ?? die sieht sehr "buggy" aus
    //   - sourceCP wird nicht gebraucht
    //   - newInfo wird erzeugt aber argument info wird in membervariable gemerkt
    ...
    protected void restore() throws SharkKBException {
        if (!this.restored()) {
            ...
            Enumeration<Information> infoIter = this.cp.enumInformation();
            while (infoIter.hasMoreElements()) {
            	Information entryInfo = infoIter.nextElement();
            ...
            if (contentType.equalsIgnoreCase(CHAT_MESSAGE_ENTRY_TYPE)) {
                ...
                // some generic information, message text will be a property
                this.info = entryInfo;
                this.message = entryInfo.getProperty("salto_attachment_comment");
            }
            }
        }
    }
